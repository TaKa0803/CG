...
void Camera::Initialize() {
	...
	//親子関係の処理
	mainCamera_.SetParent(FeaturedWorldTransform_);
...
void Camera::Update() {
	...
	if (FeaturedWorldTransform_) {
		...
		if (isOnlyGetPosition) {
			CameraMotionSupport_.translate_ = FeaturedWorldTransform_->translate_;
		}
		else {
...
void Camera::SetParent(const WorldTransform* parent) {
