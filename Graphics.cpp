...
void RootSignatureManager::Initialize(ID3D12Device* device) {
	...
	Load2DRootsignature(device);
	Load3DRootsignature(device);

}

void RootSignatureManager::Load3DRootsignature(ID3D12Device* device) {
	//サイズ設定
	rootSignature3D_.resize((size_t)FillMode::kCountOfFillMode);

	//サイズ調整
	for (auto& rootsig : rootSignature3D_) {
		rootsig.resize((size_t)BlendMode::kCountOfBlendMode);
	}
...
#pragma region PSOを生成
			...
			hr = device->CreateGraphicsPipelineState(&graphicsPipelineStateDesc,
				IID_PPV_ARGS(&rootSignature3D_[fillNum][blendNum]));
			assert(SUCCEEDED(hr));
#pragma endregion
#pragma endregion
		}
...
void RootSignatureManager::Load2DRootsignature(ID3D12Device* device) {

	rootSignature2D_.resize((size_t)BlendMode::kCountOfBlendMode);
...
#pragma region PSOを生成
		...
		hr = device->CreateGraphicsPipelineState(&graphicsPipelineStateDesc,
			IID_PPV_ARGS(&rootSignature2D_[blendNum]));
		assert(SUCCEEDED(hr));
#pragma endregion
#pragma endregion
	}
