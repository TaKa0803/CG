...
void DX::Initialize(WinApp*winApp)
{
	assert(winApp);
	winApp_ = winApp;
...
#pragma region 各種機能初期化関数
void DX::InitializeDevice()
{
...
#pragma region SwapChainを生成する
	...
	swapChainDesc.Height = WinApp::window_heigth;
...
void DX::InitializeDSV()
{
#pragma region DSV	
	dsvDescriptorHeap = CreateDescriptorHeap(device.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_DSV, 1, false);

	//DepthStencilTextureをウィンドウサイズで作成
	ID3D12Resource* depthStencilResource = CreateDepthStencilTextureResource(device.Get(), WinApp::window_width, WinApp::window_heigth);
	//DSVの設定
...
#pragma region ViewportとScissor(シザー)
	...
	viewport.Height = WinApp::window_heigth;
	viewport.TopLeftX = 0;
	viewport.TopLeftY = 0;
	...
	scissorRect.top = 0;
	scissorRect.bottom = WinApp::window_heigth;
