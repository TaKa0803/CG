...
#pragma region Instancing用
InstancingGraphicsSystem::InstancingGraphicsSystem() {
}
InstancingGraphicsSystem::~InstancingGraphicsSystem() {
...
void InstancingGraphicsSystem::Initialize(ID3D12Device* device) {
...
#pragma region RootParameter 
	...
	rootParameters[3].DescriptorTable.pDescriptorRanges = descriptorRangeInstancing;						//レジスタ番号０とバインド
	rootParameters[3].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeInstancing);
...
#pragma region ShaderをCompileする
	//Shaderをコンパイルする
	IDxcBlob* vertexShaderBlob = CompileShader(L"resources/shaders/InstancingObject.VS.hlsl", L"vs_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(vertexShaderBlob != nullptr);

	IDxcBlob* pixelShaderBlob = CompileShader(L"resources/shaders/InstancingObject.PS.hlsl", L"ps_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(pixelShaderBlob != nullptr);
#pragma endregion
#pragma region DepthStencilStateの設定を行う
	//DepthStencilStateの設定
...
void InstancingGraphicsSystem::PreDraw(ID3D12GraphicsCommandList* commandList) {
	//RootSignatureを設定。PSOに設定しているけど別途設定が必要
	commandList->SetGraphicsRootSignature(rootSignature.Get());
	commandList->SetPipelineState(graphicsPipelineState[static_cast<int>(fillMode_)][static_cast<int>(blendMode_)]);
}

void InstancingGraphicsSystem::SetBlendMode(BlendMode blend) {
	if (blendMode_ != blend) {
		blendMode_ = blend;
	}
}
